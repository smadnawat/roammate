...
class AndroidPushWorker
  include Sidekiq::Worker
    ...
    def perform(reciever,alert,badges,notification,invitation,type, device_id)
      logger.info"===================#{reciever.inspect}===#{alert.inspect}==========#{badges.inspect}==============#{device_id.inspect}==========#{type.inspect}============#{invitation.inspect}======="
        ...
        options = {
              'data' => {
              'message' => ['badge'=>  badges,'alert' => alert,'invitation_id' => invitation,'notification_type' => type,"notification_id" => notification]
              },
            "time_to_live" => 108,

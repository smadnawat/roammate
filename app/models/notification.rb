      ...
      p "+++++++++++#{type}+++++++++++++++"
	    @devices.each do |device|
	       if device.device_type == "android"
	         ...
	         AndroidPushWorker.perform_async(reciever.id, @alert, badges, notification.id, invitation, type, device.device_id, image )
	       else
          (type == "Send chat") ? @category = "ACTIONABLE" : @category = nil
          p "++++++++++++++++#{@category.inspect}+++++++++++++++++++++"
	         ApplePushWorker.perform_async(reciever.id, @alert, badges, notification.id, invitation, type, device.device_id, @category)
	       end
	    end
    end
  end
